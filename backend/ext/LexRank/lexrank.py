...
class Rank(object): # Lexrank
    def get_ranks(self,graph, d=0.85):
        ...
        degree = list(range(0,matrix_size))
...
def newssumModified(url, kkma, number): # number 추가함. 몇개의 문장을 뽑을 것인지.
    lexrank=TextRank(url,kkma)
    sum = lexrank.summarize(number)
    probdict=sorted(lexrank.sent_rank_idx.items(), key=lambda i: i[1], reverse=True)
    print(probdict)
    j=0
    senlist=[]
    problist=[]
