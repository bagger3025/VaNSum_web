...
class RNN_RNN_title(BasicModule):
    def __init__(self, args, embed=None, title=True):
        super(RNN_RNN_title, self).__init__(args)
        self.model_name = 'RNN_RNN_title'
        self.args = args
        self.title = title
    ...
    def forward(self,x,doc_lens):
        ...
        for index,doc_len in enumerate(doc_lens):
            ...
            for position, h in enumerate(valid_hidden):
                ...
                s = s + torch.mm(prob,h)
                probs.append(prob)
        return torch.cat(probs).squeeze(), doc
