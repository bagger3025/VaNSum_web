...
def get_greedy_rouge_list(textlist : list, abs):
    ...
    for i, t in enumerate(textlist):
        newtextlist.append((i, t))

    while True:
        candidates = []
        for i, text in enumerate(newtextlist):
            newlist = ans_text + [text[1]]
            score = rouge_evaluator.get_scores('\n'.join(newlist), abs)
            score = score["rouge-1"]['f']
            if score > max_rouge:
                candidates.append((i, text, score))
        if len(candidates) == 0:
            break
        max_index, max_text, max_rouge = candidates[np.array([t for _, _, t in candidates]).argmax()]
        
        ans_text.append(max_text[1])
        ans.append(max_text[0])
        newtextlist.pop(max_index)
    return ans
...
def worker(files):
    ...
    for article in files:
        ...
        # greedy_rouge_list = get_greedy_rouge_list(article['article_original'][1:], article['abstractive'])
        # content&summary
        for i, line in enumerate(article["article_original"]):
            ...
            label = '1' if i - 1 in greedy_rouge_list else '0'
            if label == "1":
...
def build_dataset(args):
    ...
    trains = []
    with open(args.source_dir) as json_file:
    ...
    for i in range(args.worker_num):
        if i == args.worker_num - 1:
            groups.append(trains[i*group_size : ])
        else:
            groups.append(trains[i*group_size : (i+1)*group_size])
    for g in groups:
        print(len(g))
