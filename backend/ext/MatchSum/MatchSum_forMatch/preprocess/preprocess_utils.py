...
from transformers import BertTokenizer, RobertaTokenizer, BertModel, RobertaModel, ElectraTokenizer, ElectraModel, RobertaForMaskedLM, AutoTokenizer, AutoModel
...
def get_rouge(dec, ref):
    ...
    scores = rouge_evaluator.get_scores(dec, ref)
...
class Rouge_kor:
    ...
    def __init__(
        ...
        self.metrics = metrics[:] if metrics is not None else Rouge_kor.DEFAULT_METRICS
        for m in self.metrics:
            if m not in Rouge_kor.AVAILABLE_METRICS:
        ...
        if self.limit_length:
            if length_limit_type not in Rouge_kor.AVAILABLE_LENGTH_LIMIT_TYPES:
    ...
    def _get_word_ngrams_and_length(n, sentences):
        ...
        tokens = Rouge_kor._split_into_words(sentences)
        return Rouge_kor._get_ngrams(n, tokens), tokens, len(tokens) - (n - 1)
    ...
    def _compute_p_r_f_score(
        ...
        f1_score = Rouge_kor._compute_f_score(precision, recall, alpha)
    ...
    def _compute_ngrams(evaluated_sentences, reference_sentences, n):
        ...
        evaluated_ngrams, _, evaluated_count = Rouge_kor._get_word_ngrams_and_length(
            n, evaluated_sentences
        )
        reference_ngrams, _, reference_count = Rouge_kor._get_word_ngrams_and_length(
    ...
    def _compute_ngrams_lcs(evaluated_sentences, reference_sentences, weight_factor=1.0):
        ...
        evaluated_unigrams_dict, evaluated_count = Rouge_kor._get_unigrams(evaluated_sentences)
        reference_unigrams_dict, reference_count = Rouge_kor._get_unigrams(reference_sentences)
    ...
    def _get_scores_rouge_n(self, all_hypothesis, all_references):
        ...
        if self.apply_avg or self.apply_best:
            scores = {metric: {stat: 0.0 for stat in Rouge_kor.STATS} for metric in metrics}
        else:
            scores = {
                metric: [{stat: [] for stat in Rouge_kor.STATS} for _ in range(len(all_hypothesis))]
                ...
                if self.apply_avg:
                    ...
                    for reference in references:
                        (
                            ...
                            reference_count,
                            overlapping_ngrams,
                        ) = Rouge_kor._compute_ngrams(hypothesis, reference, n)
                    ...
                    score = Rouge_kor._compute_p_r_f_score(
    ...
    def _get_scores_rouge_l_or_w(self, all_hypothesis, all_references, use_w=False):
        ...
        if self.apply_avg or self.apply_best:
            scores = {metric: {stat: 0.0 for stat in Rouge_kor.STATS}}
        else:
            scores = {
                metric: [{stat: [] for stat in Rouge_kor.STATS} for _ in range(len(all_hypothesis))]
            }
    ...
    def _preprocess_summary_as_a_whole(self, summary):
        sentences = Rouge_kor.split_into_sentences(summary)
        ...
        summary = Rouge_kor.REMOVE_CHAR_PATTERN.sub(" ", summary.lower()).strip()


        tokens = self.tokenize_text(Rouge_kor.REMOVE_CHAR_PATTERN.sub(" ", summary))
    ...
    def _preprocess_summary_per_sentence(self, summary):
        ...
        final_sentences = []
        for sentence in sentences:
            sentence = Rouge_kor.REMOVE_CHAR_PATTERN.sub(" ", sentence.lower()).strip()


            tokens = self.tokenize_text(Rouge_kor.REMOVE_CHAR_PATTERN.sub(" ", sentence))
