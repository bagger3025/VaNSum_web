...
class ValidMetric(MetricBase):
    ...
    def evaluate(self, score):
        ...
        for i in range(batch_size):
            ...
            rouge_score = get_rouge_scores(dec, ref)
            self.ROUGE += (rouge_score[0] + rouge_score[1] + rouge_score[2]) / 3
            self.ROUGE_ALL = [t + k for t, k in zip(self.ROUGE_ALL, rouge_score)]
            self.cur_idx += 1
...
class MatchExportMetric(MetricBase):
    ...
    def get_metric(self, reset=True):
        ...
        for i, ext in enumerate(self.ext):
            sent_ids = self.data[i]['indices'][ext]
            dec = [self.data[i]['text'][j] for j in sent_ids]
            generated_summaries.append("\n".join(dec))
