...
fname = args.xml_name#"{}wiki-{}-pages-articles-multistream.xml".format(lcode, "20230620")
...
def sentence_segment(text):
    ...
    if lcode in ['ja', 'zh']:
        ...
        sents = regex.split("([.?!])?[\n]+|[.?!] ", text)
        sents = [sent[0].lower() + sent[1:]
                 for sent in sents if sent is not None and len(sent) > 1]
    else:
        sents = regex.split("([.?!])?[\n]+|[.?!] ", text)
    return sents
