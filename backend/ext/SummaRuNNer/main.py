...
import json
from SummaRuNNer import models, utils
import argparse
import random
import logging
import numpy
...
parser.add_argument('-embedding', type=str,
                    default='data/embedding_mecab_ko.npz')
...
parser.add_argument('-model_dir', type=str,
                    default='checkpoints/RNN_RNN_seed_1_mecababs.pt')
parser.add_argument('-test_dir', type=str, default='data/test.json')
# TextFile to be summarized
parser.add_argument('-filename', type=str, default='x.txt')
...
def train():
    ...
    t2 = time()
    logging.info('Total Cost: %dh %dm %ds %d' % (
        (t2-t1)//3600, (t2-t1) // 60 % 60, int(t2-t1) % 60, int((t2-t1)*1000) % 1000))
...
def test():
    ...
    if use_gpu:
        ...
        checkpoint = torch.load(
            args.load_dir, map_location=lambda storage, loc: storage)
...
def predict(examples, preloaded=None):
    """
    predict input text
    examples = ["document1", "document2", ...]
    document = "sentence1\nsentence2\n..."
    """
    vocab, net = preloaded if preloaded is not None else pre_predict()
    ...
    for batch in tqdm(pred_iter):
        ...
        for doc_len in doc_lens:
            stop = start + doc_len
            prob = probs[start:stop].topk(doc_len)
            topk_indices = prob[1].cpu().data.tolist()
            topk_probs = prob[0].cpu().data.tolist()

            summaries.append({"index": topk_indices, "probs": topk_probs})
            start = stop
