...
# test
parser.add_argument('-model_dir',type=str,default='checkpoints/RNN_RNN_seed_1_mecababs.pt')
...
def train():
    ...
    t2 = time()
    logging.info('Total Cost: %dh %dm %ds %d'%((t2-t1)//3600, (t2-t1) // 60 % 60, int(t2-t1) % 60, int((t2-t1)*1000) % 1000))
...
# predict input text
# examples = ["document1", "document2", ...] w/ document is "sentence1\nsentence2\n..."
def predict(examples, preloaded = None):

    vocab, net = preloaded if preloaded != None else pre_predict()
