...
def validate(args, device_id, pt, step):
    ...
    tokenizer = BertTokenizer.from_pretrained('klue/bert-base', do_lower_case=True, cache_dir=args.temp_dir)
...
def test_abs_preload(args, pt):
    ...
    symbols = {'BOS': args.special_tokens["bos_id"], 'EOS': args.special_tokens["eos_id"],
                'PAD': args.special_tokens["pad_id"], 'EOQ': args.special_tokens["sent_split_id"]}    
    predictor = build_predictor(args, args.tokenizer, symbols, model, logger)
    return predictor
...
def test_text_abs(args, device_id, pt, step):
    ...
    test_iter = data_loader.Dataloader(args, load_dataset(args, 'test', shuffle=False),
                                       ...
                                       shuffle=False, is_test=True)
    tokenizer = BertTokenizer.from_pretrained('klue/bert-base', do_lower_case=True, cache_dir=args.temp_dir)
    #tokenizer = AutoTokenizer.from_pretrained("klue/roberta-large", cache_dir=args.temp_dir, return_token_type_ids=False )
    tgt_bos = '[MBOS]'
    tgt_eos = '[MEOS]'
    tgt_sent_split = '[SPL]'
...
def train_abs_single(args, device_id):
    ...
    # train_from 은 train_abs에서 빈 문자열임.
    if args.train_from != '':
    ...
    # train_from은 train_abs에서 빈 문자열임.
