...
class Translator(object):
    ...
    def translate_onebatch(self, batch):
        with torch.no_grad():
            ...
            pred_str_list, gold_str_list, src_list = [], [], []

            for trans in translations:
                pred, gold, src = trans
                replace_str = ['[PAD]', '[UNK]', '[CLS]', '[SEP]', '[MASK]', '[ unused2 ]', '[ unused2]', '[unused2 ]']
                for i in range(2000):
                    if i == 2:
                        continue
                    replace_str.append('[unused{0}]'.format(i))
                for i in range(len(replace_str)):
                    pred = pred.replace(replace_str[i], '')
                pred_str = pred.replace(r' +', ' ').replace(' [unused2] ', '<q>').replace('[unused2]', '').strip()
                ...
                gold_str_list.append(gold_str)
                src_list.append(src.strip())
        return pred_str_list, gold_str_list, src_list
    ...
    def translate(self,
        ...
        for batch in data_iter:
            pred_str_list, gold_str_list, src_list = self.translate_onebatch(batch)
            for pred_str, gold_str, src in pred_str_list, gold_str_list, src_list:
    ...
    def _fast_translate_batch(self,
        ...
        for step in range(max_length):
            ...
            # curr_score = [1, beam_size * vocab 크기] => score
            # topk_scores = [top 5개 점수]
            # topk_ids = [top 5개 ids]
