...
if __name__ == '__main__':
    ...
    parser.add_argument("-task", default=None, type=str, choices=['df', 'train_bert', 'test_bert', 'demo_bert'])
    ...
    if args.task == 'df': # and valid_df
        ...
        for i in range(len(df)):
            print(i)
            row = df.iloc[i]
            extractive_sents.append(list(np.array(row["article_original"])[row["extractive"]]))
            # print(np.array(row["article_original"]))
            # print(row["extractive"])
            # print(list(np.array(row["article_original"])[row["extractive"]]))
            # df['extractive_sents'][i] = list(np.array(row["article_original"])[row["extractive"]])
        # df['extractive_sents'] = df.apply(lambda row: row[row['extractive']], axis=1)
        # df['extractive_sents'] = df.apply(lambda row: list(np.array(row['article_original'])[row['extractive']]) , axis=1)
        df["extractive_sents"] = extractive_sents
